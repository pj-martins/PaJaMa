...
namespace FreeYouTubeDownloader.Downloader.Tasks
{
  public sealed class MultifileDownloadTask : DownloadTask
  {
    ...
    {
      Log.Trace("CALL MultifileDownloadTask.Abort()", (Exception) null);
    ...
    {
      Log.Trace("CALL MultifileDownloadTask.Pause()", (Exception) null);
    ...
    {
      Log.Trace("CALL MultifileDownloadTask.Resume()", (Exception) null);
    ...
    private void OnSinglefileDownloadStateChanged(object sender, DownloadState state)
    {
      lock (this._locker)
      {
        Log.Trace("EVENT MultifileDownloadTask.OnSinglefileDownloadStateChanged", (Exception) null);
    ...
    {
      Log.Trace("EVENT MultifileDownloadTask.OnSinglefileDownloadFinished", (Exception) null);
      lock (this._locker)
      {
        if (this.IsUsed)
          return;
    ...
    {
      Log.Trace("EVENT MultifileDownloadTask.OnSinglefileDownloadError", (Exception) null);
...
label_17:
    ...
    {
      Log.Trace("CALL MultifileDownloadTask.UpdateAfterDeserialization()", (Exception) null);
      foreach (DownloadTask downloadTask in this._singlefileDownloads.Where<SinglefileDownloadTask>((Func<SinglefileDownloadTask, bool>) (download => !download.IsCompleted)))
