...
var GridViewComponent = (function () {
    ...
    GridViewComponent.prototype.getSortedData = function (data) {
        ...
        if (this.grid.columns) {
            for (var _i = 0, _a = this.grid.getDataColumns(); _i < _a.length; _i++) {
                var col = _a[_i];
                if (col.fieldName && col.sortDirection !== undefined && col.sortDirection != gridview_1.SortDirection.None) {
        ...
        data.sort(function (a, b) {
            for (var i = 0; i < sorts.length; i++) {
                ...
                if (aval == bval)
                    continue;
                if (curr.sortDirection == gridview_1.SortDirection.Desc)
    ...
    GridViewComponent.prototype.columnOrderChanged = function (columnOrder) {
        var sourceCol;
        var targetCol;
        for (var _i = 0, _a = this.grid.columns; _i < _a.length; _i++) {
            var col = _a[_i];
        ...
        for (var _b = 0, _c = this.grid.columns; _b < _c.length; _b++) {
            var col = _c[_b];
        ...
        if (!sourceCol)
            throw columnOrder.sourceIdentifier + " not found!";
        if (!targetCol)
            throw columnOrder.targetIdentifier + " not found!";
        var targetIndex = targetCol.columnIndex;
        if (sourceCol.columnIndex <= targetCol.columnIndex) {
            for (var _d = 0, _e = this.grid.columns; _d < _e.length; _d++) {
                var col = _e[_d];
            ...
            for (var _f = 0, _g = this.grid.columns; _f < _g.length; _f++) {
                var col = _g[_f];
                if (col.getIdentifier() == sourceCol.getIdentifier())
                    continue;
                if (col.columnIndex < sourceCol.columnIndex && col.columnIndex >= targetCol.columnIndex)
        ...
        var sortedColumns = [];
        for (var _h = 0, _j = this.grid.columns; _h < _j.length; _h++) {
            var c = _j[_h];
