...
import { GridView, DataColumn, FieldType, PagingType } from './gridview';
    ...
    <div *ngIf='parentGridView.pageSize > 0 && (parentGridView.pagingType == pagingType.Auto ? parentGridViewComponent.unpagedData.length : parentGridView.totalRecords) > parentGridView.pageSize'>
		<div>
			...
			<div class="jump-to-page">
				Jump To: <input type="number" [(ngModel)]="parentGridView.currentPage" (ngModelChange)="setPage(parentGridView.currentPage)" />
			</div>
		</div>
    </div>
	<div *ngIf="(parentGridView.pagingType == 0 ? parentGridViewComponent.unpagedData.length : parentGridView.totalRecords) > 10">
