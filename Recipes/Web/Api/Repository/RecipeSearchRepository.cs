...
namespace PaJaMa.Recipes.Web.Api.Repository
{
	public class RecipeSearchRepository : Repository<RecipesDtoMapper, Recipe, RecipeCoverDto>
	{
			...
			bool bookmarked, int? recipeSourceID, bool picturesOnly)
		{
			...
			sbSQL.AppendLine("select r.RecipeID from Recipe r");
			if (userId != null)
			{
...
");
			...
			bool bookmarked, int? recipeSourceID, bool picturesOnly, int page, int pageSize, out int count)
		{
			...
			using (var cmd = recipesContext.Database.Connection.CreateCommand())
			{
				cmd.CommandText = qryParams.Item1;
				cmd.Parameters.AddRange(qryParams.Item2);
				recipesContext.Database.Connection.Open();
				...
				recipesContext.Database.Connection.Close();
			}
			count = ids.Count();
