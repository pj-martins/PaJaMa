...
namespace Crawler.Crawlers
{
    ...
    {
        protected override string baseURL => "http://www.foodnetwork.com/recipes/a-z/";
        protected override string recipesXPath => "//li[@class='m-PromoList__a-ListItem']//a";
        protected override string directionsXPath => "//div[@class='o-Method__m-Body']";
        protected override string servingsXPath => "//section[@class='o-RecipeInfo o-Yield']//dl//dd";
        protected override string ingredientsXPath => "//li[@class='o-Ingredients__a-ListItem']//label";
        protected override string imagesXPath => "//img[@class='o-AssetMultiMedia__a-Image']";
        protected override PageNumbers pageNumberURLRegex => new PageNumbers() { MaxPageRegexPattern = "<a class=\"o\\-Pagination__a\\-Button.*?\">(.*?)</a>", URLFormat = "/p/{0}" };

        protected override List<string> getKeywordPages(HtmlDocument document)
        {
            var nodes = document.DocumentNode.SelectNodes("//li[@class='o-IndexPagination__a-ListItem']//a");
            return nodes.Select(n => n.Attributes["href"].Value).ToList();
        }

        protected override float? getRating(HtmlNode node)
        {
            var match = Regex.Match(node.InnerHtml, "title=\"(.*?) of 5 stars\"");
            if (match.Success)
                return Convert.ToSingle(match.Groups[1].Value);
            return base.getRating(node);
        }
