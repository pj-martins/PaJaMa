...
namespace Crawler.Crawlers
{
	...
	public class ChowCrawler : CrawlerBase
	{
		...
		protected override string baseURL
		{
			get
			{
				return "http://www.chowhound.com/recipes/";
			}
		}
		...
		protected override string imagesXPath
		{
			get
			{
				return "//div[@data-original-2x]";
			}
		}
		protected override string getImageURL(HtmlNode node)
		{
			return node.Attributes["data-original-2x"].Value;
		}

		protected override List<string> getKeywordPages(HtmlDocument document)
		{
			return document.DocumentNode.SelectNodes("//li[contains(@class, 'act_filter')]//a")
				.Select(n => baseURL + n.InnerText).ToList();
		}
	}
}
